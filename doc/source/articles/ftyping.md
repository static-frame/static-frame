...
This function processes a signal table from an Open Source Asset Pricing (OSAP https://www.openassetpricing.com) dataset (Firm Level Characteristics / Individual / Predictors). The table has three columns: security identifier (labelled "permno"), year and month (labelled "yyyymm"), and the signal (with a name specific to the signal).
