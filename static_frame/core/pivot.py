...
def extrapolate_column_fields(
    ...
    if columns_fields_len == 1 and data_fields_len == 1:
        ...
        if not func_fields:
            ...
            sub_columns = [
                group + (field, label) for field in data_fields for label in func_fields
            ]
    ...
    *,
    ...
    if group_depth == 1:
        ...
        index = index_constructor(
            tuple(label) for label in blocks._extract_array(column_key=group_key)
        )
...
def pivot_core(
    *,
    ...
    if not columns_fields:  # group by is only index_fields
        columns = (
            data_fields
            if (func_no or func_single)
            else tuple(product(data_fields, func_fields))
        )
    ...
    group_key: int | tp.List[int] = (
        columns_fields_iloc if len(columns_fields_iloc) > 1 else columns_fields_iloc[0]
    )
...
def pivot_derive_constructors(
    *,
    ...
    return PivotDeriveConstructors(
