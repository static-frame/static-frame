...
def _extract_end_archive(file: tp.IO[bytes]) -> TEndArchive:
    ...
    data = file.read()
    if (
        len(data) == _END_ARCHIVE_SIZE
        and data[0:4] == _END_ARCHIVE_STRING
...
def yield_zinfos(
    ...
    while total < size_cd:
        ...
        filename = filename_bytes.decode(
            'utf-8' if (flags & _MASK_UTF_FILENAME) else 'cp437'
        )
        ...
        if filename_only:
            ...
            while len(extra) >= 4:  # pragma: no cover
                ...
                if ln + 4 > len(extra):
                    raise BadZipFile(
                        f'Corrupt extra field {tp:04x} (size={ln})'
                    )  # pragma: no cover
                if tp == 1:
                    ep = extra[4 : ln + 4]
                    try:
        ...
        total = (
            total + _CENTRAL_DIR_SIZE + filename_length + extra_length + comment_length
        )
...
class ZipFileRO:
    ...
    def open(self, name: str) -> tp.IO[bytes]:
        ...
        try:
            ...
            if (
                zinfo.flag_bits & _MASK_COMPRESSED_PATCH
