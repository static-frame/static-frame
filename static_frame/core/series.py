...
class Series(ContainerOperand):
    ...
    def from_overlay(cls,
        ...
        '''Return a new :obj:`Series` made by overlaying containers, aligned values are filled with values from subsequent containers with right-to-left precedence. Values are filled based on a passed function that must return a Boolean array. By default, that function is `isna_array`, returning True for missing values (NaN and None).
