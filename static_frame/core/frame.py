...
from static_frame.core.json_np_encoder import NpDtypeMapper
...
if tp.TYPE_CHECKING:
    ...
    from xarray import (
        Dataset,
    )  # pylint: disable=W0611 #pragma: no cover #type: ignore [attr-defined]
...
class Frame(ContainerOperand):
        ...
        *,
        ...
        for f in frames:
            if isinstance(f, Frame):
                ...
                index_to_frame = (
                    IndexAutoFactory if (axis == 0 and index is not None) else None
                )
                # hstack, Series will be col, if columns provided, do not evaluate name as columns
                columns_to_frame = (
                    IndexAutoFactory if (axis == 1 and columns is not None) else None
                )
                normalized.append(
                    f.to_frame(
                        axis,
        ...
        *,
        ...
        def gen() -> tp.Iterator[tp.Tuple[tp.Hashable, IndexBase]]:
            ...
            if axis == 0 and (
                ...
                or isinstance(index_constructor, IndexDefaultFactory)
            ):
                yield_elements = False
            elif axis == 1 and (
                columns_constructor is None
                or isinstance(columns_constructor, IndexDefaultFactory)
            ):
                yield_elements = False
    ...
    def from_overlay(
        ...
        *,
        ...
        fill_arrays = (
            dict()
        )  # NOTE: we will hash to NaN and NaT, but can assume we are using the same instance
        ...
        *,
        ...
        def blocks() -> tp.Iterator[np.ndarray]:
            for col_idx, (k, v) in enumerate(pairs):
                ...
                column_type = (
                    None if get_col_dtype is None else get_col_dtype(col_idx)
    ...
    def from_element_items(
        ...
        *,
        ...
        if axis is None:
            ...
            items = (
                ((index._loc_to_iloc(k[0]), columns._loc_to_iloc(k[1])), v)
                for k, v in items
            )
        ...
        *,
        ...
        try:
            ...
            if index_depth == 0:
                ...
                default_constructor = (
                    partial(Index, dtype=get_col_dtype(0)) if get_col_dtype else Index
                )
    ...
    def from_json(
        ...
        *,
        ...
        return cls.from_dict_records(
            data, name=name, dtypes=dtypes, consolidate_blocks=consolidate_blocks
        )
    ...
    def from_delimited(
        ...
        *,
        ...
        if delimiter != delimiter_native:
            ...
            def file_like() -> tp.Iterator[str]:
                if isinstance(fp, str):
                    ...
                    for row in csv.reader(
                        fp, delimiter=delimiter, quotechar=quote_char
                    ):
                        yield delimiter_native.join(row)
        ...
        if columns_depth == 0:
            ...
            if columns_depth == 1:
                ...
                if columns_continuation_token is not CONTINUATION_TOKEN_INACTIVE:
                    labels = zip_longest(
                        *(
                            store_filter.to_type_filter_iterable(x)
                            for x in columns_arrays
                        ),
        ...
        return cls(index=index, own_index=own_index, **kwargs)
    ...
    def from_csv(
        ...
        *,
        ...
        index_name_depth_level: tp.Optional[DepthLevelSpecifier] = None,
        index_constructors: IndexConstructors = None,
        index_continuation_token: tp.Union[
            tp.Hashable, None
        ] = CONTINUATION_TOKEN_INACTIVE,
        columns_depth: int = 1,
        columns_name_depth_level: tp.Optional[DepthLevelSpecifier] = None,
    ...
    def from_msgpack(msgpack_data: bin) -> "Frame":
        ...
        def decode(
            obj: dict,  # dict produced by msgpack-python
            ...
            if b"sf" in obj:
                ...
                if issubclass(cls, Frame):
                    ...
                    index_constructors = [
                        cls_map[clsname]
                        for clsname in unpackb(obj[b"index_constructors"])
                    ]
                    blocks = unpackb(obj[b"blocks"])
                ...
                if typename in ["datetime64", "timedelta64", ">m8", ">M8"]:
                    ...
                    array = np.array(
                        list(map(element_decode, data)), dtype=DTYPE_OBJECT
                    )
    ...
    def __init__(
        ...
        *,
        ...
        if data.__class__ is TypeBlocks:  # PERF: no sublcasses supported
            ...
            def blocks_constructor(
                shape: tp.Tuple[int, ...]
            ) -> None:  # pylint: disable=E0102
                if shape[0] > 0 and shape[1] > 0:
        ...
        row_count, col_count = (
            self._blocks._shape if not blocks_constructor else (None, None)
        )
        ...
        if self._COLUMNS_CONSTRUCTOR.STATIC != self._columns.STATIC:
            raise ErrorInitFrame(
                f"supplied column constructor does not match required static attribute: {self._COLUMNS_CONSTRUCTOR.STATIC}"
            )
        # -----------------------------------------------------------------------
        # index assignment
    ...
    def _reindex_other_like_iloc(
        ...
        if nm_row and not nm_column:
            ...
            if is_series:
                v = value.reindex(
                    self._columns._extract_iloc(column_key), fill_value=fill_value
                )
        elif not nm_row and nm_column:
            # only row is multi selection, reindex by index
            if is_series:
                v = value.reindex(
                    self._index._extract_iloc(row_key), fill_value=fill_value
                )
    ...
    def relabel_flat(self, index: bool = False, columns: bool = False) -> "Frame":
        """
        {doc}
    ...
    def relabel_level_drop(self, index: int = 0, columns: int = 0) -> "Frame":
        """
        {doc}
        ...
        *,
        ...
        if index_target.depth == 1:
            ...
            if remain_columns == 0:
                ...
                new_target = target_hctor._from_type_blocks(
                    remain_blocks, name=remain_labels
                )
        ...
        if axis == 0:  # select from index, remove from index
            ...
            index = Index.from_labels(
                chain(new_labels, extend_labels), name=self._index.name
            )
            columns = new_target
    ...
    def rehierarch(
        ...
        if index:
            index, index_iloc = rehierarch_from_index_hierarchy(
                labels=self._index, depth_map=index, name=self._index.name
            )
        else:
            index = self._index
            index_iloc = None
        ...
        if columns:
            columns, columns_iloc = rehierarch_from_index_hierarchy(
                labels=self._columns, depth_map=columns, name=self._columns.name
            )
            own_columns = True
        else:
            columns = self._columns
    ...
    def dropna(
        self, axis: int = 0, condition: tp.Callable[[np.ndarray], bool] = np.all
    ) -> "Frame":
        """
    ...
    def _fill_missing(
        ...
        if hasattr(value, "__iter__") and not isinstance(value, str):
            if not isinstance(value, Frame):
                raise RuntimeError(
                    "unlabeled iterables cannot be used for fillna: use a Frame"
                )
    ...
    # ---------------------------------------------------------------------------
    @doc_inject(selector="fillna")
    def fillna_leading(self, value: tp.Any, *, axis: int = 0) -> "Frame":
        """
    ...
    @doc_inject(selector="fillna")
    def fillna_trailing(self, value: tp.Any, *, axis: int = 0) -> "Frame":
        """
    ...
    @doc_inject(selector="fillna")
    def fillfalsy_leading(self, value: tp.Any, *, axis: int = 0) -> "Frame":
        """
    ...
    @doc_inject(selector="fillna")
    def fillfalsy_trailing(self, value: tp.Any, *, axis: int = 0) -> "Frame":
        """
    ...
    @doc_inject(selector="fillna")
    def fillna_forward(self, limit: int = 0, *, axis: int = 0) -> "Frame":
        """
    ...
    @doc_inject(selector="fillna")
    def fillna_backward(self, limit: int = 0, *, axis: int = 0) -> "Frame":
        """
    ...
    @doc_inject(selector="fillna")
    def fillfalsy_forward(self, limit: int = 0, *, axis: int = 0) -> "Frame":
        """
    ...
    @doc_inject(selector="fillna")
    def fillfalsy_backward(self, limit: int = 0, *, axis: int = 0) -> "Frame":
        """
    ...
    def _compound_loc_to_iloc(
        self, key: GetItemKeyTypeCompound
    ...
    def _extract_bloc(self, key: Bloc2DKeyType) -> Series:
        ...
        index = Index(
            ((self._index[x], self._columns[y]) for x, y in coords), dtype=DTYPE_OBJECT
        )
    ...
    def _axis_window(
        ...
        *,
        ...
        yield from (
            x
            for _, x in self._axis_window_items(
                size=size,
                axis=axis,
                step=step,
                window_sized=window_sized,
    ...
    def sort_index(
        ...
        *,
        ...
        key: tp.Optional[
            tp.Callable[[IndexBase], tp.Union[np.ndarray, IndexBase]]
        ] = None,
    ) -> "Frame":
        """
    ...
    def sort_values(
        ...
        *,
        ...
        key: tp.Optional[
            tp.Callable[
                [tp.Union["Frame", Series]], tp.Union[np.ndarray, "Series", "Frame"]
            ]
        ] = None,
    ) -> "Frame":
        """
    ...
    def unset_index(
        ...
        *,
        ...
        if self._columns.depth > 1:
            ...
            columns_default_constructor = partial(
                self._COLUMNS_HIERARCHY_CONSTRUCTOR._from_type_blocks, own_blocks=True
            )
    ...
    def sample(
        ...
        *,
        ...
        if columns is not None:
            columns, columns_key = self._columns._sample_and_key(
    ...
    def loc_min(self, *, skipna: bool = True, axis: int = 0) -> Series:
        """
        ...
        if axis == 0:
            return Series(
                self.index.values[post], index=immutable_index_filter(self._columns)
            )
        return Series(self.columns.values[post], index=self._index)

    @doc_inject(selector="argminmax")
    def iloc_min(self, *, skipna: bool = True, axis: int = 0) -> Series:
        """
    ...
    @doc_inject(selector="argminmax")
    def loc_max(self, *, skipna: bool = True, axis: int = 0) -> Series:
        """
    ...
    @doc_inject(selector="argminmax")
    def iloc_max(self, *, skipna: bool = True, axis: int = 0) -> Series:
        """
    ...
    def _join(
        ...
        *,
        ...
        target_left = TypeBlocks.from_blocks(
            arrays_from_index_frame(self, left_depth_level, left_columns)
        ).values
        ...
        for (k, v), left_loc_element in zip(map_iloc.items(), left_loc):
            ...
            if is_many:
                many_loc.extend(
                    Pair(p) for p in product((left_loc_element,), right_loc_part)
                )
        ...
        if join_type is Join.INNER:
            ...
            if is_many:
                extend = (
                    PairLeft((x, cifv)) for x in left_index if x not in left_loc_set
                )
            ...
            if is_many:
                extend = (
                    PairRight((cifv, x)) for x in right_index if x not in right_loc_set
                )
                final_index = Index(chain(many_loc, extend))
            else:
        ...
        final = FrameGO(
            tb, index=Index(final_index_left), columns=left_columns, own_data=True
        )
    ...
    def to_pairs(
        self, axis: int = 0
    ) -> tp.Iterable[tp.Tuple[tp.Hashable, tp.Iterable[tp.Tuple[tp.Hashable, tp.Any]]]]:
        """
        ...
        def encode(
            ...
            if package == "static_frame":
                if isinstance(obj, Frame):
                    ...
                    return {b"sf": clsname, b"blocks": packb(obj._blocks)}
                ...
                if obj.__class__ is np.ndarray:
                    if obj.dtype.type == np.object_:
                        ...
                        return {b"np": True, b"dtype": "object_", b"data": packb(data)}
    ...
    def _to_frame(self, constructor: tp.Type["Frame"]) -> "Frame":
        return constructor(
            self._blocks.copy(),
            index=self.index,
            columns=self._columns,
    ...
    def to_rst(self, config: tp.Optional[DisplayConfig] = None) -> str:
        """
    ...
    def to_markdown(self, config: tp.Optional[DisplayConfig] = None) -> str:
        """
    ...
    def to_latex(self, config: tp.Optional[DisplayConfig] = None) -> str:
        """
    ...
    def to_json_split(self, indent: int = 4):
        """
    ...
    def to_json_records(self, indent: int = 4):
        """
...
class FrameGO(Frame):
    ...
    __slots__ = ("_blocks", "_columns", "_index", "_name")
    ...
    def __setitem__(
        self, key: tp.Hashable, value: tp.Any, fill_value: tp.Any = np.nan
    ) -> None:
        """For adding a single column, one column at a time."""
        if key in self._columns:
            raise RuntimeError(
                f"The provided key ({key}) is already defined in columns; if you want to change or replace this column, use .assign to get new Frame"
            )
        ...
        if isinstance(value, Series):
            ...
            if value.ndim != 1:
                raise RuntimeError(f"can only use setitem with 1D containers")
    ...
    def extend(
        self, container: tp.Union["Frame", Series], fill_value: tp.Any = np.nan
    ) -> None:
    ...
    def __call__(
        ...
        *,
        ...
        if isinstance(self.key, tuple):
            ...
            key = (
                self.key[0],  # type: ignore [index]
                key_to_ascending_key(
                    self.key[1], self.container.shape[1]  # type: ignore [index]
                ),
            )
        else:
            key = (self.key, None)
...
class FrameAsType:
    ...
    def __init__(self, container: Frame, column_key: GetItemKeyType) -> None:
        self.container = container
...
class FrameHE(Frame):
    ...
    def __eq__(self, other: tp.Any) -> bool:
        ...
        """
        return self.equals(
            other,  # type: ignore [no-any-return]
            compare_name=True,
            compare_dtype=False,
            compare_class=False,
            skipna=True,
