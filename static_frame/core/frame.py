...
from static_frame.core.archive_npy import NPYFrameConverter, NPZFrameConverter
...
from static_frame.core.container_util import (
    ...
    axis_window_items,
    bloc_key_normalize,
    constructor_from_optional_constructors,
    df_slice_to_arrays,
    frame_to_frame,
    get_col_dtype_factory,
    get_col_fill_value_factory,
    index_constructor_empty,
...
from static_frame.core.node_fill_value import InterfaceFillValue, InterfaceFillValueGO
from static_frame.core.node_iter import (
    ...
    IterNodeGroupAxis,
    IterNodeGroupOtherReducible,
    IterNodeWindowReducible,
)
...
from static_frame.core.pivot import pivot_derive_constructors, pivot_index_map
...
from static_frame.core.util import (
    ...
    TPathSpecifierOrTextIOOrIterator,
    TSortKinds,
    TTupleCtor,
    TUFunc,
    WarningsSilent,
...
class Frame(ContainerOperand, tp.Generic[TVIndex, TVColumns, tp.Unpack[TVDtypes]]):
    ...
    def from_series(
        ...
        *,
        ...
        return cls(
            TypeBlocks.from_blocks(series.values),
            index=series.index,
            columns=(series.name,),
            name=name,
        ...
        *,
        ...
        if shape[0] > 0 and shape[1] > 0:
            raise ErrorInitFrame(
                'Cannot create zero-sized Frame from sized index and columns.'
            )
        ...
        if axis == 1:  # stacks columns (extends rows horizontally)
            ...
            if columns is IndexAutoFactory:
                ...
                try:
                    ...
                    raise ErrorInitFrame(
                        'Column names after horizontal concatenation are not unique; supply a columns argument or IndexAutoFactory.'
                    ) from e
                own_columns = True

            if index is IndexAutoFactory:
                raise ErrorInitFrame(
                    'for axis 1 concatenation, index must be used for reindexing row alignment: IndexAutoFactory is not permitted'
                )
            elif index is None:
            ...
            if index is IndexAutoFactory:
                ...
                try:
                    ...
                    raise ErrorInitFrame(
                        'Index names after vertical concatenation are not unique; supply an index argument or IndexAutoFactory.'
                    ) from e
                own_index = True

            if columns is IndexAutoFactory:
                raise ErrorInitFrame(
                    'for axis 0 concatenation, columns must be used for reindexing and column alignment: IndexAutoFactory is not permitted'
                )
            elif columns is None:
    ...
    def from_concat_items(
        ...
        *,
        ...
        def gen() -> tp.Iterator[tp.Tuple[TLabel, IndexBase]]:
            ...
            yield_elements = True
            if axis == 0 and (
                index_constructor is None
                or isinstance(index_constructor, IndexDefaultConstructorFactory)
            ):
                yield_elements = False
            elif axis == 1 and (
                columns_constructor is None
                or isinstance(columns_constructor, IndexDefaultConstructorFactory)
            ):
                yield_elements = False
        ...
        if axis == 0:
            ...
            if columns_constructor is not None:
                raise NotImplementedError(
                    'using columns_constructor for axis 0 not yet supported'
                )
            ...
            if index_constructor is not None:
                raise NotImplementedError(
                    'using index_constructor for axis 1 not yet supported'
                )
            kwargs = dict(columns=ih)
        else:
            raise AxisInvalid(f'invalid axis: {axis}')

        return cls.from_concat(
            frames,
            axis=axis,
            union=union,
            name=name,
        ...
        *,
        ...
        if columns is None:
            ...
            columns = index_from_optional_constructor(
                columns, default_constructor=cls._COLUMNS_CONSTRUCTOR
            )
        ...
        *,
        ...
        if records.__class__ is np.ndarray:
            if dtypes is not None:
                raise ErrorInitFrame(
                    'specifying dtypes when using NP records is not permitted'
                )
            return cls(
                records,
                index=index,
        ...
        if isinstance(row_reference, Series):
            raise ErrorInitFrame(
                'Frame.from_records() does not support Series records. Use Frame.from_concat() instead.'
            )
        if isinstance(row_reference, dict):
            raise ErrorInitFrame(
                'Frame.from_records() does not support dictionary records. Use Frame.from_dict_records() instead.'
            )
        ...
        def get_value_iter(col_key: TLabel, col_idx: int) -> tp.Iterator[tp.Any]:
            ...
            if get_col_fill_value is not None and get_col_dtype is not None:
                return (
                    row.get(
                        col_key,
                        get_col_fill_value(col_idx, np.dtype(get_col_dtype(col_idx))),
                    )  # might be dtype specifier
                    for row in rows_iter
                )

            if get_col_fill_value is not None:
                return (
                    row.get(col_key, get_col_fill_value(col_idx, None))
                    for row in rows_iter
                )
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for col_idx, (k, v) in enumerate(pairs):
                ...
                if v.__class__ is np.ndarray:
                    ...
                    if index is None:
                        raise ErrorInitFrame(
                            'can only consume Series in Frame.from_items if an Index is provided.'
                        )
                    ...
                    raise ErrorInitFrame(
                        'Frames are not supported in from_items constructor.'
                    )
                else:
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for col_idx, v in enumerate(fields):
                ...
                if v.__class__ is np.ndarray:
                    ...
                    if index is None:
                        raise ErrorInitFrame(
                            'can only consume Series in Frame.from_fields if an Index is provided.'
                        )
                    ...
                    raise ErrorInitFrame(
                        'Frames are not supported in from_fields constructor.'
                    )
    ...
    def _structured_array_to_d_ia_cl(
        ...
        *,
        ...
        names = (
            array.dtype.names
        )  # using names instead of fields, as this is NP convention
    ...
    @classmethod
    def _from_data_index_arrays_column_labels(
        cls,
        data: TypeBlocks,
        /,
        *,
        ...
        if columns_depth <= 1:
            ...
            columns_default_constructor = partial(
                cls._COLUMNS_HIERARCHY_CONSTRUCTOR.from_labels_delimited, delimiter=' '
            )
        ...
        *,
        ...
        if axis is None:
            if not is_dtype_specifier(dtype):
                raise ErrorInitFrame(
                    'cannot provide multiple dtypes when creating a Frame from element items and axis is None'
                )
        ...
        *,
        ...
        try:
            ...
            if index_depth > 0:
                ...
                if dtypes is None:
                    get_col_dtype = get_col_dtype_factory(
                        dict(label_to_dtype[:index_depth]), labels_index
                    )
                else:
            ...
            if index_depth == 0:
                ...
                def default_constructor(
                    ...
                    blocks = [
                        iterable_to_array_1d(it, get_col_dtype(i))[0]
            ...
            return cls.from_records(
                ...
                dtypes=dtypes
                if dtypes is not None
                else dict(label_to_dtype[index_depth:]),
                name=name,
                own_columns=own_columns,
                index_constructor=index_constructor,
                consolidate_blocks=consolidate_blocks,
    ...
    def from_delimited(
        ...
        *,
            ...
            if columns_depth == 1:
                columns, own_columns = index_from_optional_constructors(
                    ...
                    default_constructor=partial(
                        cls._COLUMNS_CONSTRUCTOR, name=columns_name
                    ),
                    explicit_constructors=columns_constructors,  # cannot supply name
                )
        ...
        if columns_select:
            if index_depth:
                raise ErrorInitFrame(
                    'Cannot use columns_select if index_depth is greater than zero.'
                )
        ...
        if index_depth:
            if index_column_first:
                ...
                if columns is not None:
                    raise ErrorInitFrame(
                        'Cannot use index_column_first if columns_depth is greater than 0.'
                    )
                elif isinstance(index_column_first, INT_TYPES):
            ...
            if index_column_first:
                raise ErrorInitFrame(
                    'Cannot set index_column_first without setting nonzero index_depth.'
                )
    ...
    def from_pandas(
        ...
        *,
        ...
        if not isinstance(value, pandas.DataFrame):
            raise ErrorInitFrame(
                f'from_pandas must be called with a Pandas DataFrame object, not: {type(value)}'
            )

        get_col_dtype = (
            None
            if dtypes is None
            else get_col_dtype_factory(
                dtypes,
    ...
    def from_parquet(
        ...
        *,
        ...
        if columns_select and index_depth != 0:
            raise ErrorInitFrame(
                f'cannot load index_depth {index_depth} when columns_select is specified.'
            )
        ...
        if columns_select:
            ...
            if len(table.column_names) < len(columns_select):
                ...
                raise ErrorInitFrame(
                    f'cannot load all columns in columns_select: missing {missing}'
                )
    ...
    def __init__(
        ...
        *,
        ...
        if data.__class__ is TypeBlocks:
            ...
            def blocks_constructor(shape: tp.Tuple[int, int]) -> None:
                if shape[0] > 0 and shape[1] > 0:
                    ...
                    raise RuntimeError(
                        'must supply a non-default value for constructing a Frame with non-zero size.'
                    )
        ...
        row_count, col_count = (
            self._blocks.shape  # pyright: ignore
            if blocks_constructor is _NA_BLOCKS_CONSTRCTOR
            else (None, None)
        )
        ...
        if own_columns:
            ...
            try:
                self._columns = index_from_optional_constructor(
                    columns,
                    default_constructor=self._COLUMNS_CONSTRUCTOR,
                    explicit_constructor=columns_constructor,
                )
        ...
        if self._COLUMNS_CONSTRUCTOR.STATIC != self._columns.STATIC:  # pyright: ignore
            raise ErrorInitFrame(
                f'Supplied `columns_constructor` does not match required static attribute: {self._COLUMNS_CONSTRUCTOR.STATIC}'
            )
    ...
    def iter_group_other_array(self) -> IterNodeGroupOtherReducible[TFrameAny]:
        ...
        return IterNodeGroupOtherReducible(
            container=self,
            function_values=partial(self._axis_group_other, as_array=True),
            function_items=partial(self._axis_group_other_items, as_array=True),
            yield_type=IterNodeType.VALUES,
            apply_type=IterNodeApplyType.SERIES_ITEMS_GROUP_VALUES,
        )
    ...
    def _reindex_other_like_iloc(
        ...
        if nm_row and not nm_column:
            ...
            if is_series:
                ...
                v = value.reindex(
                    self._columns._extract_iloc(col_key_many), fill_value=fill_value
                )
            ...
            if is_series:
                ...
                v = value.reindex(
                    self._index._extract_iloc(row_key_many), fill_value=fill_value
                )
        ...
        if v is None:
            raise RuntimeError(
                f'cannot assign {value.__class__.__name__} with key configuration: {nm_row}, {nm_column}'
            )
        return v
    ...
    def reindex(
        ...
        *,
        ...
        if columns is not None:
            if not own_columns:
                columns_owned = index_from_optional_constructor(
                    columns, default_constructor=self._COLUMNS_CONSTRUCTOR
                )
            else:
            ...
            if check_equals and self._columns.equals(columns):
                ...
                columns_ic = IndexCorrespondence.from_correspondence(
                    self._columns, columns_owned
                )
        ...
        if is_fill_value_factory_initializer(fill_value):
            get_col_fill_value = get_col_fill_value_factory(
                fill_value, columns=columns_owned
            )
            return self.__class__(
        ...
        return self.__class__(
            TypeBlocks.from_blocks(
                self._blocks.resize_blocks_by_element(
                    index_ic=index_ic, columns_ic=columns_ic, fill_value=fill_value
                ),
                shape_reference=(len(index), len(columns_owned)),  # type: ignore
            ),
            index=index,
    ...
    def relabel_flat(self, index: bool = False, columns: bool = False) -> tp.Self:
        """
        {doc}
    ...
    def relabel_level_drop(self, index: int = 0, columns: int = 0) -> tp.Self:
        """
        {doc}
    ...
    def relabel_shift_out(
        ...
        *,
        ...
        if index_target.depth == 1:
            ...
            label_src: tp.Tuple[TName] = (
                index_target.name
                if index_target._name_is_names()  # type: ignore
            ...
            if isinstance(depth_level, INT_TYPES):
                ...
                remain_labels = tuple(
                    label for i, label in enumerate(label_src) if i != depth_level
                )
            else:
                new_labels = (label_src[i] for i in depth_level)
                remain_labels = tuple(
                    label for i, label in enumerate(label_src) if i not in depth_level
                )

            target_tb = index_target._blocks  # type: ignore
            add_blocks = target_tb._slice_blocks(None, depth_level, False, True)
            ...
            if remain_columns == 0:
                ...
                new_target = target_ctor(
                    column_1d_filter(remain_blocks._blocks[0]), name=remain_labels[0]
                )
            else:
        ...
        if axis == 0:  # select from index, remove from index
            blocks = TypeBlocks.from_blocks(chain(add_blocks, self._blocks._blocks))
            index = new_target
    ...
    def _fill_missing(
        ...
        if isinstance(value, Frame):
            ...
            fill = value.reindex(
                index=self.index, columns=self.columns, fill_value=fill_value
            ).values
    ...
    # ---------------------------------------------------------------------------
    @doc_inject(selector='fillna')
    def fillna_leading(self, value: tp.Any, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillna_leading(value, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    @doc_inject(selector='fillna')
    def fillna_trailing(self, value: tp.Any, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillna_trailing(value, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    @doc_inject(selector='fillna')
    def fillfalsy_leading(self, value: tp.Any, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillfalsy_leading(value, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    @doc_inject(selector='fillna')
    def fillfalsy_trailing(self, value: tp.Any, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillfalsy_trailing(value, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    @doc_inject(selector='fillna')
    def fillna_forward(self, limit: int = 0, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillna_forward(limit=limit, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    @doc_inject(selector='fillna')
    def fillna_backward(self, limit: int = 0, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillna_backward(limit=limit, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    @doc_inject(selector='fillna')
    def fillfalsy_forward(self, limit: int = 0, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillfalsy_forward(limit=limit, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    @doc_inject(selector='fillna')
    def fillfalsy_backward(self, limit: int = 0, /, *, axis: int = 0) -> tp.Self:
        """
        ...
        """
        return self.__class__(
            self._blocks.fillfalsy_backward(limit=limit, axis=axis),
            index=self._index,
            columns=self._columns,
            name=self._name,
            own_data=True,
    ...
    def _extract(  # pyright: ignore
        ...
        if blocks_shape[0] == 0 or blocks_shape[1] == 0:
            ...
            if axis_nm[0]:  # if row not multi
                ...
                return Series(blocks.values[0], index=index, name=name_column)
            ...
            if axis_nm[1]:  # if column key is not multi
                return Series(
                    column_1d_filter(blocks._blocks[0]), index=index, name=name_column
                )
    ...
    def _extract_bloc(self, key: TBlocKey) -> TSeriesAny:
        ...
        index_constructors = list(
            chain(index.index_types.values, columns.index_types.values)
        )
        ...
        name = (
            None
            if index.name is None or columns.name is None
            else index.names + columns.names
        )
    ...
    def _extract_iloc_mask(self, key: TILocSelectorCompound) -> TFrameAny:
        ...
        return self.__class__(
            masked_blocks, columns=self._columns, index=self._index, own_data=True
        )
    ...
    def _extract_getitem_masked_array(
        self, key: TLocSelectorCompound
    ) -> MaskedArray[tp.Any, tp.Any]:
        key_iloc = self._compound_loc_to_getitem_iloc(key)
        return self._extract_iloc_masked_array(key=key_iloc)
    ...
    def _ufunc_binary_operator(
        ...
        *,
        ...
        if isinstance(other, Frame):
            ...
            return self.__class__(
                self_tb._ufunc_binary_operator(operator=operator, other=other_tb),
                index=index,
                columns=columns,
                own_data=True,
                own_index=True,
    ...
    def _axis_tuple(
        ...
        *,
        ...
        if constructor is None:
            ...
            if issubclass(constructor, tuple) and hasattr(constructor, '_make'):
    ...
    def _axis_group_final_iter(
        self,
        *,
        axis: int,
        as_array: bool,
        group_iter: tp.Iterator[
            tp.Tuple[TLabel, slice | TNDArrayAny, TypeBlocks | TNDArrayAny]
        ],
        index: IndexBase,
        columns: IndexBase,
        ...
        if as_array:
            ...
            for group, selection, tb in group_iter:
                ...
                if axis == 0:
                    ...
                    columns_group = (
                        columns._extract_iloc(selection)
                        if ordering is None
    ...
    def _axis_group_loc(
        ...
        *,
        ...
        yield from (
            x
            for _, x in self._axis_group_loc_items(
                key=key,
                axis=axis,
                drop=drop,
                as_array=as_array,
    ...
    def _axis_group_other_items(
        ...
        *,
        ...
        if group_source_ndim > 1:
            ...
            group_source_cols = [
                group_source[NULL_SLICE, i] for i in range(group_source.shape[1])
            ]
        try:
            if group_source_ndim > 1:
    ...
    def _axis_group_other(
        ...
        *,
        ...
        yield from (
            x
            for _, x in self._axis_group_other_items(
                axis=axis,
                as_array=as_array,
                group_source=group_source,
            )
    ...
    def _iter_element_loc(
        self,
        axis: int = 0,
    ) -> tp.Iterator[tp.Any]:
        yield from (x for _, x in self._iter_element_loc_items(axis=axis))
    ...
    def sort_columns(
        ...
        *,
        ...
        Args:
            ...
            {key}
        """
        if key is None and self.columns._sort_status is SortedStatus.from_ascending(
            ascending
        ):
            return self.__copy__()
        ...
        *,
        ...
        key: tp.Optional[
            tp.Callable[
                ...
                tp.Union[TNDArrayAny, TSeriesAny, TFrameAny],
            ]
        ] = None,
    ) -> tp.Self:
        """
        ...
        if axis == 0:  # get a column ordering based on one or more rows
            ...
            if key:
                ...
                cfs_is_array = cfs.__class__ is np.ndarray
                if (cfs.ndim == 1 and len(cfs) != self.shape[0]) or (
                    cfs.ndim == 2 and cfs.shape[0] != self.shape[0]
                ):
                    raise RuntimeError(
                        'key function returned a container of invalid length'
                    )
        ...
        *,
        ...
        for idx, arg in enumerate(args):
            ...
            if isinstance(arg, Series):
                if axis is None:
                    raise RuntimeError(
                        'cannot use a Series argument without specifying an axis'
                    )
                ...
                raise RuntimeError(
                    'only Series or Frame are supported as iterable lower/upper arguments'
                )
    ...
    def unset_index(
        ...
        *,
        ...
        if drop is True and names:
            raise RuntimeError(
                'Cannot specify `names` when `drop=True`, as the index will not be added back as columns.'
            )
        ...
        if drop:
            ...
            if len(names) != index_depth:
                raise RuntimeError(
                    'Passed `names` must have a label (or sequence of labels) per depth of index.'
                )

            if columns_depth > 1:
                if isinstance(names[0], str) or not hasattr(names[0], '__len__'):
                    raise RuntimeError(
                        f'Invalid name labels ({names[0]!r}); provide a sequence with a label per columns depth.'
                    )
                ...
                if index_depth == 1:
                    ...
                    columns_labels = TypeBlocks.from_blocks(  # type: ignore
                        concat_resolved(
                            (np.array([name]), self._columns.values_at_depth(i))
                        )
    ...
    def set_columns_hierarchy(
        ...
        *,
        ...
        if drop:
            blocks = TypeBlocks.from_blocks(blocks_src._drop_blocks(row_key=index_iloc))
    ...
    def unset_columns(
        ...
        *,
        ...
        if drop is True and names:
            raise RuntimeError(
                'The `names` parameter cannot be used with `drop=True` because the column labels will not be included in the resulting Frame.'
            )
        ...
        if drop is True:
            ...
            if len(names) != columns_depth:
                raise RuntimeError(
                    'Passed `names` must have a label (or sequence of labels) per depth of columns.'
                )
            ...
            if index_depth > 1:
                if isinstance(names[0], str) or not hasattr(names[0], '__len__'):
                    raise RuntimeError(
                        f'Invalid name labels ({names[0]!r}); provide a sequence with a label per index depth.'
                    )
                ...
                if columns_depth == 1:
                    ...
                    index_labels = TypeBlocks.from_blocks(  # type: ignore
                        concat_resolved(
                            (np.array([name]), self._index.values_at_depth(i))
                        )
                        for i, name in enumerate(names[0])  # type: ignore
                    )
                else:
                ...
                index_default_constructor = partial(
                    IndexHierarchy._from_type_blocks, own_blocks=True
                )
            else:
        ...
        *,
        ...
        blocks = TypeBlocks.from_blocks(
            self._blocks._shift_blocks_fill_by_element(
                row_shift=shift_index, column_shift=shift_column, wrap=True
            )
        )
        ...
        *,
        ...
        if not asc_is_element:
            ...
            if len(ascending) != shape[opposite_axis]:
                raise RuntimeError(
                    f'Multiple ascending values must match length of axis {opposite_axis}.'
                )
    ...
    def sample(
        ...
        *,
        ...
        if columns is not None:
            columns_idx, columns_key = self._columns._sample_and_key(
                count=columns, seed=seed
            )
            own_columns = True
        else:
            columns_idx = self._columns
    ...
    def loc_min(self, *, skipna: bool = True, axis: int = 0) -> TSeriesAny:
        """
    ...
    @doc_inject(selector='argminmax')
    def iloc_min(self, *, skipna: bool = True, axis: int = 0) -> TSeriesAny:
        """
    ...
    @doc_inject(selector='argminmax')
    def loc_max(self, *, skipna: bool = True, axis: int = 0) -> TSeriesAny:
        """
    ...
    @doc_inject(selector='argminmax')
    def iloc_max(self, *, skipna: bool = True, axis: int = 0) -> TSeriesAny:
        """
    ...
    def iloc_notna_first(self, *, fill_value: int = -1, axis: int = 0) -> TSeriesAny:
        """
    ...
    def iloc_notna_last(self, *, fill_value: int = -1, axis: int = 0) -> TSeriesAny:
        """
    ...
    # ---------------------------------------------------------------------------
    def iloc_notfalsy_first(self, *, fill_value: int = -1, axis: int = 0) -> TSeriesAny:
        """
    ...
    def iloc_notfalsy_last(self, *, fill_value: int = -1, axis: int = 0) -> TSeriesAny:
        """
        ...
        *,
        ...
        for field in chain(index_fields, columns_fields):
            if field not in self._columns:
                raise ErrorInitFrame(
                    f'cannot create a pivot Frame from a field ({field}) that is not a column'
                )
        if not data_fields:
    ...
    def _insert(
        ...
        *,
        ...
        blocks = TypeBlocks.from_blocks(
            chain(
                self._blocks._slice_blocks(None, slice(0, key), False, True),
                blocks_insert,
                self._blocks._slice_blocks(None, slice(key, None), False, True),
            ),
            own_data=True,
        )
    ...
    def to_pairs(
        self, *, axis: int = 0
    ) -> tp.Iterable[tp.Tuple[TLabel, tp.Iterable[tp.Tuple[TLabel, tp.Any]]]]:
        """
        ...
        return tuple(
            zip(major, (tuple(zip(minor, v)) for v in self._blocks.axis_values(axis)))
        )
        ...
        *,
        ...
        def arrays() -> tp.Iterator[TNDArrayAny]:
            for array, dtype in zip(
                ...
                if (
                    dtype.kind == DTYPE_DATETIME_KIND
    ...
    def to_xarray(self) -> 'Dataset':
        ...
        if index.depth == 1:
            ...
            coords = {
                index_name[d]: ufunc_unique1d(index.values_at_depth(d))
                for d in range(index.depth)
            }
            ...
            def columns_arrays() -> tp.Iterator[TNDArrayAny]:
                ...
                for c in self.iter_series(axis=0):  # type: ignore
                    ...
                    for index_labels, value in c.items():
                        ...
                        insert_pos = [
                            coords_index[k]._loc_to_iloc(label)
    ...
    def to_json_split(
        ...
        *,
        ...
        """
        d = dict(
            columns=JSONFilter.encode_iterable(self._columns),
    ...
    def to_json_typed(
        ...
        *,
        ...
        index = (
            None
            if self._index._map is None  # type: ignore
            else JSONFilter.encode_iterable(self._index)
        )
        columns = (
            None
            if self._columns._map is None  # type: ignore
                ...
                if include_index:
                    ...
                    if include_index_name:
                        ...
                        for col_idx in range(index_depth):
                            row.append(
                                f'{columns_names[row_idx]}' if col_idx == 0 else ''
                            )
                    else:
...
class FrameGO(Frame[TVIndex, TVColumns]):
    ...
    def __setitem__(
        ...
        if key in self._columns:
            raise RuntimeError(
                f'The provided key ({key!r}) is already defined in columns; if you want to change or replace this column, use `Frame.assign` to get new `Frame`.'
            )
        ...
        if isinstance(value, Series):
            ...
            if len(value) != row_count:
                raise RuntimeError(
                    f'incorrectly sized unindexed value: {len(value)} != {row_count}'
                )
            block = value.values
...
class FrameAssignILoc(FrameAssign):
    ...
    def __call__(
        ...
        *,
        ...
        if is_series:
            assigned = self.container._reindex_other_like_iloc(
                value,
                key,
                is_series=is_series,
                is_frame=is_frame,
            ...
            assigned = self.container._reindex_other_like_iloc(
                ...
                is_frame=is_frame,
                fill_value=fill_value,
            )._blocks._blocks  # pyright: ignore
...
class FrameAssignBLoc(FrameAssign):
    ...
    def __call__(
        ...
        *,
        ...
        key = bloc_key_normalize(key=self.key, container=self.container)
        if is_series:
...
class FrameAsType:
    ...
    def __init__(self, container: TFrameAny, column_key: TILocSelector) -> None:
        self.container = container
        ...
        *,
        ...
        if self.column_key.__class__ is slice and self.column_key == NULL_SLICE:
            ...
            if not is_dtype_specifier(dtypes):
                raise RuntimeError(
                    'must supply a single dtype specifier if using a column selection other than the NULL slice'
                )
