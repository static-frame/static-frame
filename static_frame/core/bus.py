...
class Bus(ContainerBase, StoreClientMixin, tp.Generic[TVIndex]):
    ...
    def _update_mutable_persistent_one(
        self,
        key: TILocSelector,
    ) -> None:
    ...
    def _update_mutable_persistant_many(
        self,
        key: TILocSelector,
    ) -> None:
    ...
    def _update_mutable_max_persist_one(
        self,
        key: TILocSelector,
    ) -> None:
        """
    ...
    def _update_mutable_max_persist_many(
        self,
        key: TILocSelector,
    ) -> None:
        """Load all `Frame` targeted by `key`."""
        ...
        if labels_unloaded.any():
            ...
            to_load_count = len(labels_to_load)  # pyright: ignore
    ...
    def values(self) -> TNDArrayObject:
        ...
        if max_persist is None:  # load all at once if possible
            post = np.fromiter(
                self._update_mutable_persistent_iter(),
                dtype=DTYPE_OBJECT,
                count=self.__len__(),
            )
        else:
            post = np.fromiter(
                self._update_mutable_max_persist_iter(),
                dtype=DTYPE_OBJECT,
                count=self.__len__(),
            )
        post.flags.writeable = False
    ...
    def display(
        ...
        *,
        ...
        return self._to_series_state()._display(
            config,
    ...
    def shapes(self) -> Series[TVIndex, np.object_]:
        ...
        """
        values = (
            f.shape if f is not FrameDeferred else None for f in self._values_mutable
        )
    ...
    def nbytes(self) -> int:
        ...
        return sum(
            f.nbytes if f is not FrameDeferred else 0 for f in self._values_mutable
        )
    ...
    def status(self) -> Frame[TVIndex, tp.Any, *tuple[tp.Any, ...]]:
        ...
        def gen() -> tp.Iterator[Series[TVIndex, tp.Any]]:
            ...
            for attr, dtype, missing in (
                ...
                values = (
                    getattr(f, attr) if f is not FrameDeferred else missing
                    for f in self._values_mutable
                )
