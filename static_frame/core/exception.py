...
class RelabelInvalid(RuntimeError):
    def __init__(self, *args: tp.Any) -> None:
        super().__init__(
            'Relabelling with an unordered iterable is not permitted.', *args
        )
...
class BatchIterableInvalid(RuntimeError):
    def __init__(self, *args: tp.Any) -> None:
        super().__init__(
            'Batch iterable does not yield expected pair of label, container.', *args
        )
...
class InvalidDatetime64Comparison(RuntimeError):
    def __init__(self, *args: tp.Any) -> None:
        super().__init__(
            'Cannot perform set operations on datetime64 of different units; use astype to align units before comparison.',
            *args,
        )
...
class GrowOnlyInvalid(RuntimeError):
    def __init__(self, *args: tp.Any) -> None:
        super().__init__(
            'Cannot perform an in-place grow-only operation due to the class of the columns Index.'
        )
...
class NotImplementedAxis(NotImplementedError):
    def __init__(self, *args: tp.Any) -> None:
        super().__init__(
            'Iteration along this axis is too inefficient; create a consolidated Frame with Quilt.to_frame()',
            *args,
        )
...
class ErrorNotTruthy(ValueError):
    def __init__(self, *args: tp.Any) -> None:
        super().__init__(
            'The truth value of a container is ambiguous. For a truthy indicator of non-empty status, use the `size` attribute.',
            *args,
        )
