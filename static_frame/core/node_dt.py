...
class InterfaceDatetime(Interface, tp.Generic[TVContainer_co]):
    ...
    def _validate_dtype_non_str(
        ...
        """
        if (
            dtype.kind == DTYPE_DATETIME_KIND or dtype == DTYPE_OBJECT
    ...
    def _validate_dtype_str(
        ...
        """
        if (
            dtype.kind in DTYPE_STR_KINDS or dtype == DTYPE_OBJECT
        ) and dtype not in exclude:
            return
        raise RuntimeError(f'invalid dtype ({dtype}) for operation on string types')

    def _fill_missing_dt64(
        self,
        ...
        if targets.any():
            if self._fill_value is FILL_VALUE_DEFAULT:
                raise RuntimeError(
                    'Cannot convert NaT: provide a `fill_value` to `via_dt()`.'
                )
            else:
    ...
    def _fill_missing_element_method(
        self,
        array: TNDArrayAny,
        *,
        method_name: str,
    ...
    def _fill_missing_element_attr(
        self,
        array: TNDArrayAny,
        *,
        attr_name: str,
            ...
            for block in self._blocks:
                ...
                if block.dtype.kind == DTYPE_DATETIME_KIND:
                    ...
                    array = self._fill_missing_element_attr(
                        array=block, attr_name='year', dtype=DTYPE_INT_DEFAULT
                    )
                yield array
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for block in self._blocks:
                ...
                if block.dtype.kind == DTYPE_DATETIME_KIND:
                    ...
                    array = self._fill_missing_element_attr(
                        array=block, attr_name='month', dtype=DTYPE_INT_DEFAULT
                    )
                yield array
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for block in self._blocks:
                self._validate_dtype_non_str(
                    block.dtype, exclude=self.DT64_EXCLUDE_YEAR_MONTH
                )
                if block.dtype.kind == DTYPE_DATETIME_KIND:
                    ...
                    array = self._fill_missing_element_attr(
                        array=block, attr_name='day', dtype=DTYPE_INT_DEFAULT
                    )
                yield array
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for block in self._blocks:
                ...
                if block.dtype.kind == DTYPE_DATETIME_KIND:
                    ...
                    array = self._fill_missing_element_attr(
                        array=block, attr_name='hour', dtype=DTYPE_INT_DEFAULT
                    )
                yield array
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for block in self._blocks:
                ...
                if block.dtype.kind == DTYPE_DATETIME_KIND:
                    ...
                    array = self._fill_missing_element_attr(
                        array=block, attr_name='minute', dtype=DTYPE_INT_DEFAULT
                    )
                yield array
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for block in self._blocks:
                ...
                if block.dtype.kind == DTYPE_DATETIME_KIND:
                    ...
                    array = self._fill_missing_element_attr(
                        array=block, attr_name='second', dtype=DTYPE_INT_DEFAULT
                    )
                yield array
    ...
    def timetuple(self) -> TVContainer_co:
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for block in self._blocks:
                ...
                self._validate_dtype_non_str(
                    block.dtype, exclude=self.DT64_EXCLUDE_YEAR_MONTH_SUB_MICRO
                )
                ...
                array = self._fill_missing_element_method(
                    array=block, method_name='timetuple', args=(), dtype=DTYPE_OBJECT
                )
                yield array
        ...
        def blocks() -> tp.Iterator[TNDArrayAny]:
            for block in self._blocks:
                ...
                if array_dt64.dtype in self.DT64_EXCLUDE_YEAR_MONTH_SUB_MICRO:
                    raise RuntimeError(
                        f'invalid derived dtype ({array_dt64.dtype}) for iso format'
                    )
...
class InterfaceBatchDatetime(InterfaceBatch):
    ...
    def __call__(
        ...
        *,
        ...
        Args:
            fill_value: If NAT are encountered, use this value.
        """
        return self.__class__(self._batch_apply, fill_value=fill_value)
    ...
    def year_month(self) -> 'Batch':
        ...
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).year_month
        )
    ...
    def year_quarter(self) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).year_quarter
        )
    ...
    def weekday(self) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).weekday()
        )
    ...
    def quarter(self) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).quarter()
        )
    ...
    def is_month_end(self) -> 'Batch':
        """Return Boolean indicators if the day is the month end."""
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).is_month_end()
        )

    def is_month_start(self) -> 'Batch':
        """Return Boolean indicators if the day is the month start."""
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).is_month_start()
        )

    def is_year_end(self) -> 'Batch':
        """Return Boolean indicators if the day is the year end."""
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).is_year_end()
        )

    def is_year_start(self) -> 'Batch':
        """Return Boolean indicators if the day is the year start."""
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).is_year_start()
        )

    def is_quarter_end(self) -> 'Batch':
        """Return Boolean indicators if the day is the quarter end."""
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).is_quarter_end()
        )

    def is_quarter_start(self) -> 'Batch':
        """Return Boolean indicators if the day is the quarter start."""
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).is_quarter_start()
        )
    ...
    def timetuple(self) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).timetuple()
        )
    ...
    def isoformat(self, *, sep: str = 'T', timespec: str = 'auto') -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).isoformat(
    ...
    def fromisoformat(self) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).fromisoformat()
        )
    ...
    def strftime(self, format: str, /) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).strftime(format)
        )
    ...
    def strptime(self, format: str, /) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).strptime(format)
        )
    ...
    def strpdate(self, format: str, /) -> 'Batch':
        ...
        """
        return self._batch_apply(
            lambda c: c.via_dt(fill_value=self._fill_value).strpdate(format)
        )
