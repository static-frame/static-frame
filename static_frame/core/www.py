...
class WWW:
    ...
    def _url_prepare(url: str) -> str:
        ...
        return urlunparse(url_parts._replace(path=quote(url_parts.path)))
        ...
        with ZipFile(archive) as zf:
            ...
            if component:
                ...
                if len(names) > 1:
                    ...
                    raise RuntimeError(
                        f'More than one file found in zip archive ({samples}{etc}); name a single file with the `component` argument.'
                    )
                name = names.pop()
            data = zf.read(name)
