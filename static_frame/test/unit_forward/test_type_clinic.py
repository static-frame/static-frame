...
def test_check_frame_a() -> None:
    f = sf.Frame.from_records(
        ([3, '192004', 0.3], [3, '192005', -0.4]), columns=('permno', 'yyyymm', 'Mom3m')
    )

    cr = f.via_type_clinic(
        sf.Frame[sf.Index[np.int64], sf.TIndexAny, *tuple[tp.Any, ...]]
    )
    assert len(cr) == 0
...
def test_check_index_hierarchy_a() -> None:
    ...
    cr = f.via_type_clinic(
        sf.Frame[sf.TIndexHierarchyAny, sf.TIndexAny, *tuple[tp.Any, ...]]
    )
    assert len(cr) == 0
...
def test_check_index_hierarchy_b() -> None:
    ...
    cr = f.via_type_clinic(
        sf.Frame[
            sf.IndexHierarchy[*tuple[tp.Any, ...]], sf.TIndexAny, *tuple[tp.Any, ...]
        ]
    )
    assert len(cr) == 0
