...
def subset_contiguous_sum(target: int) -> tp.Tuple[tp.Tuple[int, ...], ...]:
    ...
    @lru_cache()
    def subset_sum(
        numbers: tp.Sequence[int],
        partial: tp.Tuple[int, ...] = (),
        partial_sum: int = 0,
...
ST_TYPES_FOR_UNIQUE_MIXED = (
    ST_TYPES_FLOAT_NO_NAN + ST_TYPES_COMMON + ST_TYPES_UNARY_BINARY
)
...
def get_labels(min_size: int = 0, max_size: int = MAX_ROWS) -> st.SearchStrategy:
    """
    ...
    def gen() -> tp.Iterator[st.SearchStrategy]:
        yield st.lists(get_label(), min_size=min_size, max_size=max_size, unique=True)

        for strat in ST_TYPES_FOR_UNIQUE:
            yield st.lists(strat(), min_size=min_size, max_size=max_size, unique=True)
...
def get_shape_1d2d(
    ...
    return st.one_of(
        ...
        get_shape_1d(min_size=min_rows, max_size=max_rows),
    )
...
def get_array_object(
    shape: tp.Tuple[int, ...] = (MAX_ROWS, MAX_COLUMNS), unique: bool = True
) -> st.SearchStrategy:
    if unique:
...
def get_array_from_dtype_group(
    ...
    """
    array_object = (
        get_array_object(shape=shape, unique=unique)
        .filter(partial(array_filter, unique=unique))
        .map(array_map)
    )
    array_non_object = (
        hypo_np.arrays(get_dtype(dtype_group), shape, unique=unique)
        .filter(partial(array_filter, unique=unique))
        .map(array_map)
    )
...
def get_array_1d2d(
    *,
    ...
    if 1 in range(min_columns, max_columns + 1):
        ...
        return st.one_of(
            array_2d,
            get_array_1d(min_size=min_rows, max_size=max_rows, dtype_group=dtype_group),
        )
    return array_2d
...
def get_arrays_2d_aligned(min_size: int = 1, max_size: int = 10) -> st.SearchStrategy:
...
def get_blocks(
    ...
    def constructor(
        rows_column_widths: tp.Tuple[int, tp.Iterator[int]],
    ) -> st.SearchStrategy:
...
get_type_blocks_numeric: tp.Callable[..., st.SearchStrategy] = partial(
    get_type_blocks, dtype_group=DTGroup.NUMERIC
)
    ...
    def constructor(shape: tp.Tuple[int, int]) -> st.SearchStrategy:
        ...
        return st.tuples(
            ...
            get_array_1d2d(  # let columns be variable3
                min_rows=rows, max_rows=rows, dtype_group=dtype_group
            ),
        )
...
get_index_date: tp.Callable[..., st.SearchStrategy] = partial(
    get_index, cls=IndexDate, dtype_group=DTGroup.DATE
)
get_index_date.__name__ = 'get_index_date'

get_index_year: tp.Callable[..., st.SearchStrategy] = partial(
    get_index, cls=IndexYear, dtype_group=DTGroup.YEAR
)
...
def get_index_hierarchy(
    ...
    def constructor(
        labels_spacings: tp.Tuple[
            tp.Sequence[tp.Sequence[str]], tp.Sequence[tp.Iterable[int]]
        ],
    ) -> st.SearchStrategy:
    ...
    def get_labels_spacings(depth_size: tp.Tuple[int, int]) -> st.SearchStrategy:
        ...
        if dtype_group is not None:
            level = get_array_1d(
                min_size=size, max_size=size, unique=True, dtype_group=dtype_group
            )
        else:
...
def get_series(
    ...
    def constructor(shape: tp.Tuple[int]) -> st.SearchStrategy:
        ...
        return st.builds(
            cls,
            get_array_1d(min_size=size, max_size=size, dtype_group=dtype_group),
            index=index,
        )
...
# label index, values
get_series_date_numeric: tp.Callable[..., st.SearchStrategy] = partial(
    get_series,
...
get_series_str_dgt1_numeric: tp.Callable[..., st.SearchStrategy] = partial(
    get_series,
    min_size=1,
    dtype_group=DTGroup.NUMERIC,
...
get_series_obj_dgt1_numeric: tp.Callable[..., st.SearchStrategy] = partial(
    get_series,
    min_size=1,
    dtype_group=DTGroup.NUMERIC,
    index_cls=IndexHierarchy,
...
get_frame_date_str_numeric: tp.Callable[..., st.SearchStrategy] = partial(
    get_frame,
    dtype_group=DTGroup.NUMERIC,
    index_cls=IndexDate,
    index_dtype_group=DTGroup.DATE,
