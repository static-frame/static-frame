...
def test_series_getitem_a() -> None:
    ...
    def proc(
        x: sf.Series[sf.Index[np.str_], np.int64],
...
def test_series_getitem_b() -> None:
    ...
    def proc(
        x: sf.SeriesHE[sf.Index[np.str_], np.int64],
    ) -> sf.SeriesHE[sf.Index[np.str_], np.int64]:
        return x.dropna()
...
def test_series_drop() -> None:
    ...
    def proc2(
        x: sf.Series[sf.Index[np.str_], np.str_],
    ) -> sf.Series[sf.Index[np.str_], np.str_]:
        return x.dropna()
...
def test_series_ih1() -> None:
    s1: TSeries1 = sf.Series(
        (10, 20, np.nan), index=sf.IndexDate(('2022-01-01', '2022-01-02', '2022-01-03'))
    )
