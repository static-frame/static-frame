...
class TestUnit(TestCase):
    ...
    def test_interface_summary_b(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                ('Accessor Datetime', 23),
                ('Accessor Fill Value', 26),
                ('Accessor Hashlib', 10),
                ('Accessor Reduce', 20),
                ('Accessor Regular Expression', 7),
                ('Accessor String', 39),
                ('Accessor Transpose', 24),
                ('Accessor Type Clinic', 5),
                ('Accessor Values', 3),
                ('Assignment', 16),
                ('Attribute', 12),
                ('Constructor', 37),
    ...
    def test_interface_get_signatures_b(self) -> None:
        ...
        self.assertEqual(
            sig,
            '__init__(values, /, *, index, name, dtype, index_constructor, own_index)',
        )
        self.assertEqual(signa, '__init__()')
    ...
    def test_interface_get_signatures_d(self) -> None:
        sig, signa = _get_signatures(
            'sum', sum, delegate_func=sum, delegate_name='sum2', max_args=99
        )
    ...
    def test_interface_get_signatures_e(self) -> None:
        ...
        self.assertEqual(
            sig,
            'sum(iterable, /, start).sum2(iterable, /, start).sum3(iterable, /, start)',
        )
    ...
    def test_interface_get_signatures_f(self) -> None:
        ...
        self.assertEqual(
            sig,
            'sum(iterable, /, start).dns.sum2(iterable, /, start).sum3(iterable, /, start)',
        )
    ...
    def test_interface_get_signatures_h(self) -> None:
        # if we provide a func and no name, we assume that func is a __call__ on the parent object
        sig, signa = _get_signatures(
            'sum', sum, delegate_func=sum, terminus_func=min, max_args=99
        )
    ...
    def test_interface_get_frame_a(self) -> None:
        ...
        self.assertEqual(f1.columns.values.tolist(), ['cls_name', 'group', 'doc'])
        self.assertEqual(
    ...
    def test_interface_assign_a(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                'assign[key](value, *, fill_value)',
                'assign[key].apply(func, *, fill_value)',
                'assign[key].apply_element(func, *, dtype, fill_value)',
                'assign[key].apply_element_items(func, *, dtype, fill_value)',
                'assign.iloc[key](value, *, fill_value)',
                'assign.iloc[key].apply(func, *, fill_value)',
                'assign.iloc[key].apply_element(func, *, dtype, fill_value)',
                'assign.iloc[key].apply_element_items(func, *, dtype, fill_value)',
                'assign.loc[key](value, *, fill_value)',
                'assign.loc[key].apply(func, *, fill_value)',
                'assign.loc[key].apply_element(func, *, dtype, fill_value)',
                'assign.loc[key].apply_element_items(func, *, dtype, fill_value)',
                'assign.bloc[key](value, *, fill_value)',
                'assign.bloc[key].apply(func, *, fill_value)',
                'assign.bloc[key].apply_element(func, *, dtype, fill_value)',
                'assign.bloc[key].apply_element_items(func, *, dtype, fill_value)',
            ],
        )
    ...
    def test_interface_via_re_signature_no_args(self) -> None:
        ...
        self.assertEqual(
            inter.loc[
                inter['group'] == InterfaceGroup.AccessorFillValue, 'signature_no_args'
            ].values.tolist(),
            [
                'via_fill_value().loc',
                'via_fill_value().__getitem__()',
                'via_fill_value().via_T',
                'via_fill_value().__add__()',
                'via_fill_value().__sub__()',
                'via_fill_value().__mul__()',
                'via_fill_value().__truediv__()',
                'via_fill_value().__floordiv__()',
                'via_fill_value().__mod__()',
                'via_fill_value().__pow__()',
                'via_fill_value().__lshift__()',
                'via_fill_value().__rshift__()',
                'via_fill_value().__and__()',
                'via_fill_value().__xor__()',
                'via_fill_value().__or__()',
                'via_fill_value().__lt__()',
                'via_fill_value().__le__()',
                'via_fill_value().__eq__()',
                'via_fill_value().__ne__()',
                'via_fill_value().__gt__()',
                'via_fill_value().__ge__()',
                'via_fill_value().__radd__()',
                'via_fill_value().__rsub__()',
        ...
        self.assertEqual(
            inter.loc[
                inter['group'] == InterfaceGroup.AccessorRe, 'signature_no_args'
            ].values.tolist(),
            [
                'via_re().search()',
                'via_re().match()',
    ...
    def test_interface_summary_name_obj_iter_a(self) -> None:
        ...
        self.assertEqual(
            inter['signature_no_args'].values.tolist(),
