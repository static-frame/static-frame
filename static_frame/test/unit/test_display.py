...
from static_frame.core.display import (
    FORMAT_EMPTY,
    DisplayCell,
    DisplayTypeCategoryFactory,
    DisplayTypeInt,
    terminal_ansi,
)
from static_frame.core.display_color import HexColor
from static_frame.core.display_config import DisplayFormatLaTeX
from static_frame.test.test_case import TestCase, skip_np2, skip_win, temp_file
...
class TestUnit(TestCase):
    ...
    def test_display_config_a(self) -> None:
        ...
        d = Display.from_values(
            np.array([[1, 2], [3, 4]], dtype=np.int64), header='header', config=config
        )
    ...
    def test_display_cell_align_left_a(self) -> None:
        config_right = sf.DisplayConfig.from_default(
            cell_align_left=False, type_color=False
        )
        config_left = sf.DisplayConfig.from_default(
            cell_align_left=True, type_color=False
        )

        index = Index((x for x in 'abc'))

        self.assertEqual(
            index.display(config_left).to_rows(), ['<Index>', 'a', 'b', 'c', '<<U1>']
        )
    ...
    def test_display_cell_align_left_b(self) -> None:
        ...
        f1 = Frame.from_records(
            records, columns=('p', 'q', 'r', 's', 't'), index=('w', 'x', 'y')
        )
    ...
    def test_display_type_show_a(self) -> None:
        ...
        self.assertEqual(
            f1.display(config_type).to_rows(),
            [
                '<Frame>',
                '<Index> p       q       r     s      t      <<U1>',
                '<Index>',
    ...
    def test_display_cell_fill_width_a(self) -> None:
        ...
        self.assertEqual(
            s.display(config_width_6).to_rows(),
            [
                '<Se...',
                '<In...',
                'a      Lor...',
        ...
        config = sf.DisplayConfig.from_default(
            type_color=False, cell_max_width_leftmost=20
        )
        ...
        self.assertEqual(
            f.display(config_width_6).to_rows(),
            [
                '<Fr...',
                '<In... 0      1      2      3      <in...',
                '<In...',
    ...
    def test_display_truncate_a(self) -> None:
        ...
        config_rows_7_cols_5 = sf.DisplayConfig.from_default(
            display_rows=7, display_columns=5
        )

        size = 10000
        columns = 100
        a1 = (np.arange(size * columns)).reshape((size, columns)) * 0.001
        ...
        self.assertEqual(len(tuple(f.display(config_rows_12_cols_8).to_rows())), 15)
    ...
    def test_display_flatten_a(self) -> None:
        ...
        self.assertEqual(
            d1.flatten().to_rows(), ['header header 1 5 2 6 3 7 4 8 <int64> <int64>']
        )

        self.assertEqual(
            d1.transform().to_rows(),
...
&lt;int64&gt; &lt;int64&gt; &lt;float64&gt; &lt;bool&gt;</div>"""
    ...
    def test_display_html_table_b(self) -> None:
        ...
        f = Frame.from_records(records, columns=('p', 'q', 'r'), index=('w', 'x'))
        self.assertEqual(
            f._repr_html_(),
    ...
    def test_display_html_index_b(self) -> None:
        ...
        self.assertEqual(
            post[2], '<span style="color: #777777">&lt;IndexHierarchy: ...</span>'
        )

        post = f.display(
            sf.DisplayConfig(
                display_format=DisplayFormats.HTML_PRE, cell_max_width_leftmost=36
            )
        ).to_rows()
    ...
    def test_display_max_width_a(self) -> None:
        ...
        post = f2.display(
            sf.DisplayConfig(type_color=False, cell_max_width_leftmost=30)
        ).to_rows()
    ...
    def test_display_float_scientific_b(self) -> None:
        ...
        self.assertEqual(
            s1.display(
                sf.DisplayConfig(
                    type_color=False, value_format_complex_scientific='{:.1e}'
                )
            ).to_rows(),
            [
                '<Series>',
    ...
    def test_display_include_index_a(self) -> None:
        ...
        config2 = DisplayConfig(
            include_index=False, type_show=False, include_columns=False
        )
    ...
    def test_display_value_color_a(self) -> None:
        ...
        s2 = f1.assign[1].apply(
            lambda s: s.iter_element().apply(
                lambda e: HexColor.format_terminal('green' if e > 0 else 'blue', str(e))
            )
        )
        post = s2.display()
