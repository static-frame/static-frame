...
class TestUnit(TestCase):
    ...
    def test_index_constructors_a(self) -> None:
        ...
        for cls in all_subclasses(IndexBase):
            ...
            self.assertTrue(
                immutable_cls.STATIC, f'{immutable_cls.__name__} should be STATIC=True'
            )
            self.assertFalse(
                mutable_cls.STATIC, f'{mutable_cls.__name__} should be STATIC=False'
            )

            # Name and symmetry checks
            self.assertEqual(
                mutable_cls.__name__,
                immutable_cls.__name__ + 'GO',
