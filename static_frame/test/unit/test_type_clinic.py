...
def test_check_b():
    ...
    assert (
        ...
        == 'In Tuple[~T, ~T] ~T Expected int, provided str invalid'
    )
...
def test_check_type_c():
    ...
    TypeClinic([(3, ['a', 'b']), (4, ['x', 'y'])]).check(
        tp.List[tp.Tuple[int, tp.List[str]]]
    )
...
def test_check_result_c():
    post = TypeClinic(sf.Index(('a', 'b')))(
        tp.Annotated[sf.Index[np.str_], Require.Len(1)]
    ).to_str()
    assert (
        scrub_str(post)
        == 'In Annotated[Index[str_], Len(1)] Len(1) Expected length 1, provided length 2'
    )
...
def test_check_type_numpy_d():
    ...
    h1 = np.ndarray[
        tp.Any,
        np.dtype[np.signedinteger[np.integer[np.number[tp.Union[_16Bit, _32Bit]]]]],
    ]
    TypeClinic(a).check(h1)

    h2 = np.ndarray[
        tp.Any,
        np.dtype[np.signedinteger[np.integer[np.number[tp.Union[_16Bit, _64Bit]]]]],
    ]
    with pytest.raises(TypeError):
        TypeClinic(a).check(h2)
...
def test_check_type_tuple_c():
    ...
    assert [r[1] for r in cr] == [
        'Expected tuple length of 3, provided tuple length of 2'
    ]
...
def test_check_type_tuple_e4():
    TypeClinic(('b', 'x', 3, 4, 10, 20, 50)).check(
        tuple[str, str, tp.Unpack[tuple[int, ...]]]
    )
...
def test_check_type_tuple_e8():
    TypeClinic(('a', 'x', 1, 20, 30, False)).check(
        tuple[str, str, tp.Unpack[tuple[int, int, int]], bool]
    )
...
def test_check_type_tuple_e13():
    with pytest.raises(TypeError):
        TypeClinic(('a', 'x', 1, 20, 30, False)).check(
            tuple[float, str, tp.Unpack[tuple[int, ...]], bool, float]
        )


@skip_pyle310
def test_check_type_tuple_e14():
    TypeClinic((1.2, 'x', 20, 30, False, 4.3)).check(
        tuple[float, str, tp.Unpack[tuple[int, ...]], bool, float]
    )
...
def test_check_type_tuple_e15():
    TypeClinic((1.2, 'x', False, 4.3)).check(
        tuple[float, str, tp.Unpack[tuple[int, ...]], bool, float]
    )
...
def test_check_type_tuple_e16():
    TypeClinic((1.2, 'x', 1, 2, 3, 4, 5, 5, False, 4.3)).check(
        tuple[float, str, tp.Unpack[tuple[int, ...]], bool, float]
    )
...
def test_check_type_tuple_e17():
    with pytest.raises(TypeError):
        TypeClinic((1.2, 'x', 1, 2, 3, 4, 5, 5, False)).check(
            tuple[float, str, tp.Unpack[tuple[int, ...]], bool, float]
        )
...
def test_check_type_tuple_e19():
    with pytest.raises(TypeError):
        TypeClinic(('a', 'x', 1, False)).check(
            tuple[str, str, tp.Unpack[tuple[int, bool, int]], bool]
        )
...
def test_check_type_tuple_e22():
    TypeClinic(('a', 'x', False, 1, 5, 'b')).check(
        tuple[str, str, bool, tp.Unpack[tuple[int, int, str]]]
    )
...
def test_check_type_tuple_e23():
    with pytest.raises(TypeError):
        TypeClinic(('a', 'x', False, 1, 5, False)).check(
            tuple[str, str, bool, tp.Unpack[tuple[int, int, str]]]
        )
...
def test_check_interface_b():
    ...
    try:
        ...
        assert (
            scrub_str(str(e))
            == 'In args of (a: int, b: int) -> bool In arg b Expected int, provided str invalid'
        )
...
def test_check_interface_f1():
    @CallGuard.check
    def proc1(
        idx: tp.Annotated[sf.Index[np.str_], Require.Len(3), Require.Name('foo')],
    ) -> int:
        return len(idx)
...
def test_check_index_hierarchy_c():
    ...
    h2 = sf.IndexHierarchy[
        tp.Unpack[tp.Tuple[sf.Index[np.int_], sf.Index[np.str_], sf.Index[np.bool_]]]
    ]
...
def test_check_index_hierarchy_d1():
    ...
    v2 = sf.IndexHierarchy.from_labels(
        [
            (
                1,
                'a',
...
def test_check_index_hierarchy_d2():
    ...
    if IS_NP2:
        assert (
            scrub_str(v1.via_type_clinic(h1).to_str())
            == 'In IndexHierarchy[Index[int64], Index[str_], Unpack[Tuple[Index[bool], ...]]] Depth 1 Index[str_] Expected str_, provided bool invalid'
        )
    else:
        assert (
            scrub_str(v1.via_type_clinic(h1).to_str())
            == 'In IndexHierarchy[Index[int64], Index[str_], Unpack[Tuple[Index[bool_], ...]]] Depth 1 Index[str_] Expected str_, provided bool_ invalid'
        )


def test_check_index_hierarchy_e1():
    v1 = sf.IndexHierarchy.from_labels([(1, 3), (3, 2), (2, 3)])
    h1 = sf.IndexHierarchy[tp.Unpack[tp.Tuple[sf.Index[np.integer], ...]],]
    assert v1.via_type_clinic(h1).validated
...
def test_check_index_hierarchy_e2():
    ...
    assert (
        scrub_str(v1.via_type_clinic(h1).to_str())
        == 'In IndexHierarchy[Unpack[Tuple[Index[integer], ...]]] Tuple[Index[integer], ...] Index[integer] Expected integer, provided str_ invalid'
    )
...
def test_check_index_hierarchy_f():
    ...
    assert (
        scrub_str(v1.via_type_clinic(h1).to_str())
        == 'In IndexHierarchy[Index[integer], IndexDate, IndexDate] Expected IndexHierarchy has 3 depth, provided IndexHierarchy has 2 depth'
    )
...
def test_check_index_hierarchy_g():
    ...
    if IS_NP2:
        assert (
            scrub_str(v1.via_type_clinic(h1).to_str())
            == 'In IndexHierarchy[Index[int64], Index[str_], Unpack[Tuple[Index[bool], ...]], Index[str_], Index[int64], Index[int64]] Expected IndexHierarchy has 5 depth (excluding Unpack), provided IndexHierarchy has 4 depth'
        )
    else:
        assert (
            scrub_str(v1.via_type_clinic(h1).to_str())
            == 'In IndexHierarchy[Index[int64], Index[str_], Unpack[Tuple[Index[bool_], ...]], Index[str_], Index[int64], Index[int64]] Expected IndexHierarchy has 5 depth (excluding Unpack), provided IndexHierarchy has 4 depth'
        )
...
def test_check_frame_a1():
    ...
    h1 = sf.Frame[
        sf.IndexDate,  # type: ignore[type-arg]
        sf.Index[np.str_],
    ]
...
def test_check_frame_c():
    ...
    if IS_NP2:
        assert (
            scrub_str(TypeClinic(f3)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], Unpack[Tuple[float64, ...]]] Tuple[float64, ...] Expected float64, provided bool invalid'
        )
    else:
        assert (
            scrub_str(TypeClinic(f3)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], Unpack[Tuple[float64, ...]]] Tuple[float64, ...] Expected float64, provided bool_ invalid'
        )
...
def test_check_frame_d():
    ...
    records2 = (
        (True, 1.8, 3.1, 1.2, 'x', 'y'),
        (False, 3.2, 8.1, 3.5, 'a', 'b'),
    )
    ...
    if IS_NP2:
        assert (
            scrub_str(TypeClinic(f3)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], bool, Unpack[Tuple[float64, ...]], str_, str_] Field 0 Expected bool, provided float64 invalid'
        )
    else:
        assert (
            scrub_str(TypeClinic(f3)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], bool_, Unpack[Tuple[float64, ...]], str_, str_] Field 0 Expected bool_, provided float64 invalid'
        )
    ...
    if IS_NP2:
        assert (
            scrub_str(TypeClinic(f4)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], bool, Unpack[Tuple[float64, ...]], str_, str_] Field 1 Expected str_, provided float64 invalid'
        )
    else:
        assert (
            scrub_str(TypeClinic(f4)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], bool_, Unpack[Tuple[float64, ...]], str_, str_] Field 1 Expected str_, provided float64 invalid'
        )
...
def test_check_frame_e1():
    ...
    if IS_NP2:
        assert (
            scrub_str(TypeClinic(f3)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], Unpack[Tuple[float64, ...]], str_, str_] Fields 0 to 2 Tuple[float64, ...] Expected float64, provided bool invalid'
        )
    else:
        assert (
            scrub_str(TypeClinic(f3)(h1).to_str())
            == 'In Frame[IndexDate, Index[str_], Unpack[Tuple[float64, ...]], str_, str_] Fields 0 to 2 Tuple[float64, ...] Expected float64, provided bool_ invalid'
        )
...
def test_check_frame_e2():
    ...
    assert (
        scrub_str(f1.via_type_clinic(h1).to_str())
        == 'In Frame[IndexDate, Index[str_], Unpack[Tuple[float64, ...]], str_, str_] Field 0 Expected str_, provided float64 invalid'
    )
...
def test_check_frame_f2():
    ...
    assert (
        scrub_str(f1.via_type_clinic(h1).to_str())
        == 'In Frame[IndexDate, Index[str_], str_, str_, Unpack[Tuple[float64, ...]]] Fields 2 to 3 Tuple[float64, ...] Expected float64, provided str_ invalid'
    )
...
def test_check_frame_g():
    ...
    assert (
        scrub_str(f1.via_type_clinic(h1).to_str())
        == 'In Frame[IndexDate, Index[str_], str_, str_, str_, Unpack[Tuple[float64, ...]]] Expected Frame has 3 dtype (excluding Unpack), provided Frame has 2 dtype'
    )
...
def get_hints(
    records: tp.Union[tp.Iterable[TValidation], ClinicResult],
) -> tp.Tuple[str]:
...
def test_validate_labels_order_d2():
    ...
    assert get_hints(v._iter_errors(idx1, None, (), ())) == (
        "Expected has unmatched labels 'e'",
    )
...
def test_validate_labels_order_e1():
    ...
    if IS_NP2:
        assert get_hints(v._iter_errors(idx1, None, (), ())) == (
            "Expected labels exhausted at provided np.str_('e')",
        )
    else:
        assert get_hints(v._iter_errors(idx1, None, (), ())) == (
            "Expected labels exhausted at provided 'e'",
        )
...
def test_validate_labels_order_e8():
    ...
    assert get_hints(v._iter_errors(idx1, None, (), ())) == (
        "Expected has unmatched labels 'f'",
    )
...
def test_validate_labels_order_e9():
    ...
    assert get_hints(v._iter_errors(idx1, None, (), ())) == (
        "Expected has unmatched labels 'x', ..., 'y'",
    )
...
def test_validate_labels_order_e11():
    ...
    v = Require.LabelsOrder('a', ...)
    assert get_hints(v._iter_errors(idx1, None, (), ())) == (
        "Expected LabelsOrder('a', ...) to be used on Index or IndexHierarchy, not provided Series",
    )
...
def test_validate_labels_order_g():
    ...
    if IS_NP2:
        assert (
            scrub_str(cr.to_str())
            == "In Frame[IndexDate, Annotated[Index[str_], LabelsOrder(['a', <lambda>], ..., ['c', <lambda>])], int64, int64, bool] Annotated[Index[str_], LabelsOrder(['a', <lambda>], ..., ['c', <lambda>])] LabelsOrder(['a', <lambda>], ..., ['c', <lambda>]) Validation failed of label 'c' with <lambda>"
        )
    else:
        assert (
            scrub_str(cr.to_str())
            == "In Frame[IndexDate, Annotated[Index[str_], LabelsOrder(['a', <lambda>], ..., ['c', <lambda>])], int64, int64, bool_] Annotated[Index[str_], LabelsOrder(['a', <lambda>], ..., ['c', <lambda>])] LabelsOrder(['a', <lambda>], ..., ['c', <lambda>]) Validation failed of label 'c' with <lambda>"
        )
...
def test_validate_labels_order_h4():
    ...
    f.via_type_clinic.check(
        sf.Frame[sf.IndexDate, sf.Index[np.str_], np.int64, np.bool_, np.str_]
    )

    f.via_type_clinic.check(
        sf.Frame[
            tp.Annotated[
                sf.IndexDate, sf.Require.LabelsOrder('2022-01-03', ..., '2023-04-02')
            ],
            sf.Index[np.str_],
            np.int64,
            np.bool_,
    ...
    f.via_type_clinic.check(
        sf.Frame[
            ...
            tp.Annotated[sf.Index[np.str_], sf.Require.LabelsOrder(..., 'b', 'c')],
            np.int64,
            np.bool_,
            np.str_,
        ]
    ...
    with pytest.raises(TypeError):
        f.via_type_clinic.check(
            sf.Frame[
                tp.Annotated[
                    sf.IndexDate,
                    sf.Require.LabelsOrder('2022-01-03', ..., '2023-04-02'),
                ],
                tp.Annotated[
                    sf.Index[np.str_],
                    sf.Require.LabelsOrder(
...
def test_validate_labels_order_i2():
    ...
    v = Require.LabelsOrder(
        [..., lambda s: s.dtype.kind == 'i'], ['c', lambda s: s.dtype == bool], 'd'
    )
    assert not tuple(v._iter_errors(f.columns, None, (), (f,)))

    v = Require.LabelsOrder(
        [..., lambda s: s.dtype.kind == 'i'], ['c', lambda s: s.dtype == float], 'd'
    )
    assert tuple(v._iter_errors(f.columns, None, (), (f,)))
...
def test_validate_labels_order_i3():
    ...
    v = Require.LabelsOrder(
        [..., lambda s: s.dtype.kind == 'i'], 'c', ['d', lambda s: s.dtype.kind == 'U']
    )
    assert not tuple(v._iter_errors(f.columns, None, (), (f,)))
...
def test_validate_labels_match_h1():
    ...
    v3 = Require.LabelsMatch(
        ['b', lambda s: s.dtype.kind == 'i'], ['c', lambda s: s.dtype.kind == 'b'], 'd'
    )
...
def test_validate_labels_match_h2():
    ...
    v2 = Require.LabelsMatch(
        [{'a', 'b'}, lambda s: s.dtype.kind == 'i'],
    )
    assert not tuple(v2._iter_errors(f.columns, None, (), (f,)))
...
def test_validate_apply_a():
    ...
    assert get_hints(v2._iter_errors(idx1, None, (), ())) == (
...
def test_check_error_display_a():
    ...
    try:
        ...
        if IS_NP2:
            assert (
                scrub_str(str(e))
                == 'In Frame[IndexDate, Index[int64], int64, int64, str_] Expected str_, provided bool invalid In Frame[IndexDate, Index[int64], int64, int64, str_] Index[int64] Expected int64, provided str_ invalid'
            )
        else:
            assert (
                scrub_str(str(e))
...
def test_type_clinic_a():
    ...
    f = sf.Frame.from_records(
        ...
        columns=columns,
        index=index,
        dtypes=(np.int64, np.bool_, np.int64, np.bool_),
    )
...
def test_type_clinic_to_hint_f():
    ...
    assert (
        TypeClinic(np.array([False, True])).to_hint()
...
def test_type_clinic_to_hint_j2():
    assert (
        TypeClinic({3: 'a', 42: 'x', 1.2: 'y'}).to_hint()
        == dict[tp.Union[int, float], str]
    )

    assert (
        TypeClinic({'a': 3, 'x': 30, 'z': 10.5}).to_hint()
        == dict[str, tp.Union[int, float]]
    )


def test_type_clinic_to_hint_j3():
    assert (
        TypeClinic({3: 'a', 42: 'x', 1.2: 'y', 30: b'q'}).to_hint()
        == dict[tp.Union[int, float], tp.Union[str, bytes]]
    )

    assert (
        TypeClinic({'a': 3, 'x': 30, b'z': 10.5}).to_hint()
        == dict[tp.Union[str, bytes], tp.Union[int, float]]
    )
...
def test_type_clinic_typevar_a():
    ...
    assert (
        scrub_str(f1.via_type_clinic(h1).to_str())
        == 'In Frame[Index[~T: generic], Index[~T: generic], Unpack[Tuple[Any, ...]]] Index[~T: generic] ~T: generic Expected int64, provided str_ invalid'
    )
...
def test_type_clinic_typevar_b2():
    ...
    assert (
        scrub_str(cr.to_str())
        == 'In Frame[Index[~T: (int64, float64)], Index[~T: (int64, float64)], Unpack[Tuple[Any, ...]]] Index[~T: (int64, float64)] ~T: (int64, float64) Expected float64, provided int64 invalid'
    )
...
def test_type_clinic_typevar_c():
    ...
    assert (
        scrub_str(cr.to_str())
        == 'In Frame[Index[~T], Index[~T], Unpack[Tuple[Any, ...]]] Index[~T] ~T Expected str_, provided int64 invalid'
    )
...
def test_type_clinic_typevar_d3():
    ...
    assert (
        scrub_str(cr.to_str())
        == 'In Tuple[~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C]] ~T: Union[A, B, C] Union[A2, B1, C2] Expected A2, provided C1 invalid In Tuple[~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C]] ~T: Union[A, B, C] Union[A2, B1, C2] Expected B1, provided C1 invalid In Tuple[~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C], ~T: Union[A, B, C]] ~T: Union[A, B, C] Union[A2, B1, C2] Expected C2, provided C1 invalid'
    )
...
def test_call_guard_typevar_a():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert (
            scrub_str(str(w[0].message))
            == 'In return of (a: Series[Index[~T3], ~T4]) -> Series[Index[~T3], ~T4] Series[Index[~T3], ~T4] ~T4 Expected float64, provided str_ invalid'
        )
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        if IS_NP2:
            assert (
                scrub_str(str(w[0].message))
                == 'In return of (a: Series[Index[~T3], ~T4]) -> Series[Index[~T3], ~T4] Series[Index[~T3], ~T4] ~T4 Expected bool, provided str_ invalid'
            )
        else:
            assert (
                scrub_str(str(w[0].message))
                == 'In return of (a: Series[Index[~T3], ~T4]) -> Series[Index[~T3], ~T4] Series[Index[~T3], ~T4] ~T4 Expected bool_, provided str_ invalid'
            )
...
def test_call_guard_typevar_b():
    ...
    with warnings.catch_warnings(record=True) as w:
        _ = process1(
            sf.Series((1.2, 5.4), index=('a', 'b')),
            sf.Series((4, 5), index=np.array((30, 10), dtype=np.int64), dtype=np.int64),
        )
        assert (
            scrub_str(str(w[0].message))
            == 'In args of (a: Series[Index[~T], number[Any]], b: Series[Index[~T], number[Any]]) -> Series[Index[~T], number[Any]] In arg b Series[Index[~T], number[Any]] Index[~T] ~T Expected str_, provided int64 invalid'
        )
...
def test_call_guard_typevar_c2():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert (
            scrub_str(str(w[0].message))
            == 'In args of (x: Iterable[~T1: (int, str)], y: Iterable[~T1: (int, str)]) -> List[~T1: (int, str)] In arg x Iterable[~T1: (int, str)] ~T1: (int, str) Expected int, provided str invalid'
        )
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert (
            scrub_str(str(w[0].message))
            == 'In args of (x: Iterable[~T1: (int, str)], y: Iterable[~T1: (int, str)]) -> List[~T1: (int, str)] In arg y Iterable[~T1: (int, str)] ~T1: (int, str) Expected int, provided str invalid'
        )
...
def test_call_guard_typevar_d():
    ...
    _ = process1(
        sf.Series((1.2, 5.4), index=('a', 'b'), dtype=np.int8),
        sf.Series((4, 5), index=('a', 'b'), dtype=np.int8),
    )

    _ = process1(
        sf.Series((1.2, 5.4), index=('a', 'b'), dtype=np.uint16),
        sf.Series((4, 5), index=('a', 'b'), dtype=np.uint16),
    )
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert (
            scrub_str(str(w[0].message))
            == 'In args of (a: Series[Index[str], ~T: (uint16, int8)], b: Series[Index[str], ~T: (uint16, int8)]) -> Series[Index[str], ~T: (uint16, int8)] In arg b Series[Index[str], ~T: (uint16, int8)] ~T: (uint16, int8) Expected uint16, provided int8 invalid'
        )
...
def test_call_guard_typevar_e():
    ...
    _ = process2(
        sf.Series((1.2, 5.4), index=('a', 'b')), sf.Series((4.3, 5.1), index=('a', 'c'))
    )
