...
def test_dbquery_create_a2():
    ...
    assert (
        post
        == 'CREATE TABLE IF NOT EXISTS foo (__index0__ INTEGER, x TEXT, y INTEGER, z BOOLEAN);'
    )
...
def test_dbquery_create_a3():
    ...
    with temp_file('.db') as fp:
        ...
        dbq.execute(
            frame=f, label=f.name, include_index=False, scalars=False, eager=False
        )
...
def test_dbquery_create_a4():
    ...
    with temp_file('.db') as fp:
        conn = sqlite3.connect(fp)
        dbq = DBQuery.from_defaults(conn)
        query, parameters = dbq._sql_insert(
            frame=f,
            label=f.name,
            schema='',
            include_index=False,
...
def test_mysql_type_decl_to_dtype():
    ...
    assert mysql_type_decl_to_dtype('TIMESTAMP') == np.dtype(
        'datetime64[s]'
    )  # MySQL TIMESTAMP is always in seconds
